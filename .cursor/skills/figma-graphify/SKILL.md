...
description: >-
  ...
  a Figma file or calling Figma MCP get_design_context. Call recommend or
  resolve — never Read graph.json. Product is Resolve.
---

# Resolve

Ingest the **shared node**. Graph stays on disk (`.graphify/graph.json`).
**Do not Read that file.** Call `recommend` (or `resolve` if you know the name),
then Figma on that `figmaNodeId`. Then `verify_frame`.
